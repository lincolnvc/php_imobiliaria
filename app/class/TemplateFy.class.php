...
Class TemplateFy
{
    ...
    public function tpl( $tpl )
    {
        ...
        {
            echo "Arquivo $this->tpl não encontrado em: " . $this->tpldir . "$this->tpl !";            exit;
        }
    ...
    public function render( $printable=null )
    {
        ...
        foreach ( $this->tpldata as $item )
        {
            ...
            {
                if ( preg_match_all( "/(<!--\{$key\}-->)/i", $this->contents, $m ) )
                {
                    $this->contents = @preg_replace( '/\<!--\{' . $key . '\}-->/i', $value, $this->contents );
                }
        ...
        //set baseUri
        $this->loadUri();
        $commons = array( '/(\[baseUri\])/', '/(\[baseVersion\])/' );
        $commonr = array( "$this->baseUri", date( 'dmyhis' ) );
    ...
    private function ifetch( $data, $target, $html )
    {
        ...
        foreach ( $data as $item )
        {
            ...
            while ( list( $key, $value ) = each( $item ) )
            {
                if ( empty( $value ) || $value == "" || $value == NULL || strlen( $value ) <= 0 )
                {
                    //$value = "&nbsp;";
                }
